...
std::unique_ptr<ifrt::CompileOptions> MakeIfrtCompileOptions(
  ...
  if (platform_id != CpuId() && platform_id != CudaId() &&
    ...
    return std::make_unique<ifrt::XlaCompileOptions>(
        ...
        /*has_callbacks=*/host_callbacks.size());
  }
  ...
  auto ffi_loaded_host_callbacks_ptr =
      new FfiLoadedHostCallbacks(std::move(opaque_host_callbacks));
  std::shared_ptr<void> ffi_loaded_host_callbacks(
      ffi_loaded_host_callbacks_ptr);
  auto type_id =
      xla::ffi::TypeIdRegistry::TypeId(FfiLoadedHostCallbacks::id.type_id);
  return std::make_unique<ifrt::XlaCompileOptions>(
      std::move(options), std::move(ifrt_loaded_host_callbacks),
      /*has_callbacks=*/host_callbacks.size(),
      std::move(ffi_loaded_host_callbacks), std::move(type_id));
}
