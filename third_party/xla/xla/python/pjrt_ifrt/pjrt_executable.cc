...
absl::StatusOr<std::unique_ptr<LoadedExecutable>> PjRtLoadedExecutable::Create(
  ...
  return CreateInternal(client, std::move(pjrt_loaded_executable),
                        ...
                        has_callbacks, std::move(ffi_loaded_host_callbacks),
...
PjRtLoadedExecutable::PjRtLoadedExecutable(
    ...
    : client_(client),
      ...
      has_callbacks_(has_callbacks),
      ffi_loaded_host_callbacks_(std::move(ffi_loaded_host_callbacks)),
      ffi_loaded_host_callbacks_type_id_(ffi_loaded_host_callbacks_type_id),
...
PjRtLoadedExecutable::Execute(absl::Span<tsl::RCReference<Array>> args,
  ...
  if (platform_id == CpuId() || platform_id == CudaId() ||
    ...
    auto fn_pre = [&](xla::ffi::TypeIdRegistry::TypeId type_id, void* data) {
      LOG(INFO) << "fn_pre: " << type_id << "\n";
    };
    context->ffi_context().ForEach(fn_pre);
    CHECK_OK(context->ffi_context().Insert(ffi_loaded_host_callbacks_type_id_,
                                           ffi_loaded_host_callbacks_.get()));
  ...
  if (has_callbacks_) {
    ...
    status.OnReady([all_loaded_host_callbacks = all_loaded_host_callbacks_,
                    ...
                    ffi_loaded_host_callbacks =
                        ffi_loaded_host_callbacks_](absl::Status) mutable {
